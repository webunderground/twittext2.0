...
function updatedata() {
 ...
 if(isset($_GET['hashtag']) && $_GET['hashtag'] != '') {
	 ...
	 $result = mysql_query("SELECT * FROM `messages1` WHERE hashtag LIKE '%$tags%' ORDER BY `id` DESC");
	 $title = "Search results for <font color='red'>".$tags."</font> <a href='index.php'>clear</a>";
 } else {
   $result = mysql_query("SELECT * FROM `messages1` ORDER BY `id` DESC");
     $title = "Updates";
 }
 ?>
  <br />
          ...
          <div class="post__header">
            <div class="post__headerText">
              <h3>
                Twittext
                <span class="post__headerSpecial"
                  ><span class="material-icons post__badge"> verified </span>@twittext</span
                >
              </h3>
            </div>
            <div class="post__headerDescription">
